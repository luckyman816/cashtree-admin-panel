...
import { getLeaders } from 'store/reducers/leader';
...
const LeaderBoard = () => {
  ...
  const leader_state = useSelector((state) =>  state.leader.leaders)
  const [leaders, setLeaders]  = useState(leader_state)
  useEffect(()=>{
    setLeaders(leader_state)
  },[leader_state])
    ...
    <MainCard>
      <ScrollX>
        <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
          <ReactTable columns={columns} data={leaders} />
        </DndProvider>
      </ScrollX>
    </MainCard>
  );
