...
// types
import { dailyUsersProps } from 'types/dailyUsers';


// ----------------------------------------------------------------------

const initialState: dailyUsersProps = {
  dailyUsers : [],
  error: null
};

const dailyUsers = createSlice({
  name: 'dailyUsers',
  initialState,
  reducers: {
    // HAS ERROR
...
// Reducer
export default dailyUsers.reducer;
...
export function getDailyUsers() {
  return async () => {
    try {
      const response = await axios.get('/api/dailyUsers/getall');
      dispatch(dailyUsers.actions.putReducer(response.data));
    } catch (error) {
      dispatch(dailyUsers.actions.hasError(error));
    }
  };
}
