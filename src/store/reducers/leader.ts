...
const initialState: LeaderProps = {
  leaders : [],
  error: null
};
...
export function getLeaders() {
  return async () => {
    try {
      const response = await axios.get('/api/leaders/getall');
      dispatch(leader.actions.putReducer(response.data));
    } catch (error) {
      dispatch(leader.actions.hasError(error));
    }
  };
}
